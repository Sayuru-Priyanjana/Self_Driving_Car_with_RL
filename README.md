...
# train on the chicane map instead of the default oval
python train.py --track chicane --timesteps 400000
```

Progress is printed every 10 episodes. Reward plots are saved to
`plots/reward_curve.png` every 20 000 timesteps (or `plots/reward_curve_chicane.png`
when training with `--track chicane`).
...
# test on the chicane map (loads models/ppo_car_chicane.zip)
python test.py --track chicane --deterministic
```
...
A second, harder track is available via `--track chicane` (see Training/Testing
above): tighter corner radius (`r_out=120` vs. 140) plus an S-curve chicane cut
into the top straight, generated by `_build_chicane_track()` in `track.py`. It
reuses the same `Track` class and reward/observation logic — only the
geometry differs. Artifacts are kept separate from the default oval track
(`models/ppo_car_chicane.zip`, `logs/rewards_chicane.csv`,
`plots/reward_curve_chicane.png`) so training it never overwrites the
original model. Compare `plots/reward_curve.png` (oval) against
`plots/reward_curve_chicane.png` (chicane) to see how much harder the new
layout is for the agent.
